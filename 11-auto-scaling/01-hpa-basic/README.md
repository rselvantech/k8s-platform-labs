...
├── README.md                        # this file
├── 01-hpa-basic-anki.csv            # Anki flashcard deck
├── 01-hpa-basic-quiz.md             # standalone quiz
└── src/
    ...
    └── break-fix/
        ├── 01-hpa-missing-requests.yaml    # broken: HPA TARGETS stays `<unknown>`
        ├── 02-hpa-scaledown-disabled.yaml  # broken: HPA never scales down
        ├── 03-hpa-ambiguous-selector.yaml  # broken: 2 HPAs on same Deployment
        └── 04-hpa-wrong-target-type.yaml   # broken: HPA formula miscalculation
```
...
10× its normal traffic still needs more pods — not a bigger request. <u>VPA's histogram model
is not designed to respond to sudden traffic changes; it smooths over days of data.</u>
...
Metrics-server is the cluster addon that aggregates resource metrics from all nodes in the cluster and exposes them through a single API endpoint — the Metrics API (`metrics.k8s.io`). It is what powers `kubectl top`, HPA, and VPA.
...
```
Container level (cAdvisor — raw, no aggregation):
  Read directly from each container's cgroup.
  No pod-level or node-level rollup happens here.

Pod level (metrics-server computes this — by summation):
  Pod CPU = sum of that pod's container CPU values
  Pod memory = sum of that pod's container memory values
  (This is what kubectl top pods shows)

  Foundational fact: the PodMetrics API object has NO pod-level total
  field at all — only a `containers` array. Every consumer (kubectl top,
  the HPA controller) computes the pod total itself, every time, from
  that array. There is nowhere in the schema for metrics-server to have
  hidden a pre-summed pod value even if it wanted to.

Node level (NOT computed by metrics-server — read as one direct value):
  Node CPU/memory = the node's ROOT cgroup usage, measured directly by
  cAdvisor/kubelet and relayed as a single number via /metrics/resource.
  (This is what kubectl top nodes shows)

  Why this is NOT "sum of pods + overhead": the root cgroup is the
  common parent of every pod's cgroup AND every non-pod process on the
  node (kubelet, container runtime, sshd, etc.) — the kernel has already
  rolled all of that into one cumulative figure before metrics-server
  ever reads it. There is no separate summation step, and no separate
  "overhead" term added on — it was never split apart to begin with.

  Confirms in the schema: NodeMetrics has exactly one `usage` field
  and no per-pod breakdown of any kind — consistent with it being a
  single direct measurement, not a computed aggregate.

  Real capture from a live 3node cluster:
  { "usage": { "cpu": "178355592n", "memory": "948020Ki" } }
  ← one flat number per node, nothing to sum.

Container-level breakdown remains available at the pod level:
  PodMetrics.containers[] retains each container's individual usage
  This is what HPA's ContainerResource metric type reads
...
```
cAdvisor (per container+ per node) → metrics-server reads →
  stores as PodMetrics (per container, aggregated per pod)
  stores as NodeMetrics (per node)
```
...
Four broken scenarios below. For each: apply the configuration, read the symptom from the cluster, diagnose, then reveal the answer. Each scenario ends with an explicit cleanup step — run it before starting the next scenario.
...
```bash
kubectl apply -f 01-hpa-missing-requests.yaml
...
**Cleanup:**
```bash
kubectl delete -f 01-hpa-missing-requests.yaml --ignore-not-found
```
...
kubectl apply -f ../01-nginx-deploy.yaml
kubectl apply -f ../02-load-generator.yaml
kubectl apply -f 02-hpa-scaledown-disabled.yaml
...
**Cleanup:**
```bash
kubectl delete -f 02-hpa-scaledown-disabled.yaml --ignore-not-found
kubectl delete pod load-generator --grace-period=0 --force --ignore-not-found
kubectl delete -f ../01-nginx-deploy.yaml --ignore-not-found
```
...
kubectl apply -f 03-hpa-ambiguous-selector.yaml
...
**Cleanup:**
```bash
kubectl delete -f 03-hpa-ambiguous-selector.yaml --ignore-not-found
kubectl delete -f ../03-hpa-cpu-v2.yaml --ignore-not-found
kubectl delete -f ../01-nginx-deploy.yaml --ignore-not-found
```
...
```bash
kubectl apply -f ../01-nginx-deploy.yaml
kubectl apply -f 04-hpa-wrong-target-type.yaml
...
**Cleanup:**
```bash
kubectl delete -f 04-hpa-wrong-target-type.yaml --ignore-not-found
kubectl delete -f ../01-nginx-deploy.yaml --ignore-not-found
cd ..
```
